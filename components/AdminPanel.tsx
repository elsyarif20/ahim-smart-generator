...
<div class="wrap">
  <div class="kop">
    <div class="kop1 text-xl font-bold">AHIM SMART GENERATOR</div>
...
<div id="rekapModal" style="display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.5); z-index: 1000;">
  <div style="background: white; margin: 2% auto; padding: 20px; width: 95%; max-width: 1200px; border-radius: 10px; max-height: 90%; overflow-y: auto;">
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px; border-bottom: 2px solid #345; padding-bottom: 10px;">
      <h2 style="margin: 0; color: #345;">📊 Rekapitulasi Kinerja Karyawan</h2>
...
document.getElementById('unit').addEventListener('change', function() {
  const unit = (this as HTMLSelectElement).value;
...
function exportToWord() {
  ...
  const jabatan = (document.getElementById('jabatan') as HTMLInputElement).value;
  const unit = (document.getElementById('unit') as HTMLInputElement).value;
  const pedagogik = (document.getElementById('pedagogik') as HTMLInputElement).value;
    ...
    <body style="font-family: Arial, sans-serif; margin: 40px;">
      ...
      <div style="margin-top: 50px; display: flex; justify-content: space-between;">
        <div style="text-align: center;">
          <p>Kepala Sekolah</p>
          <br><br><br>
          <p>\${unit === 'SMP' ? 'Iswahyudin,SE' : 'Antoni Firdaus,SHI,M.Pd.'}</p>
        </div>
        <div style="text-align: center;">
          <p>Tanggal: \${tanggal}</p>
          <p>Penilai</p>
...
function exportToPNG() {
  ...
  ctx.fillText('AHIM SMART GENERATOR', canvas.width/2, 60);
  ...
  const namaKepsekPNG = unit === 'SMP' ? 'Iswahyudin,SE' : 'Antoni Firdaus,SHI,M.Pd.';
...
function exportRekapPNG() {
  ...
  ctx.fillText('AHIM SMART GENERATOR', canvas.width/2, 50);
  ctx.font = 'bold 20px Arial';
  ctx.fillText('Rekapitulasi Kinerja Karyawan 2025/2026', canvas.width/2, 90);
